...
const SlideLayout: React.FC<SlideLayoutProps> = ({ data, isActive, lang }) => {
  ...
  const trainingLabel = lang === 'de' ? 'TEIL 2 · AKTUELLE PROMPT-TECHNIK FÜR IT-ARBEIT' : 'PART 2 · CURRENT PROMPT TECHNIQUES FOR IT WORK';
  const contentLabel = lang === 'de' ? 'MISSION // BRIEFING' : 'MISSION // BRIEF';
  const compareLabel = lang === 'de' ? 'BOSS FIGHT // VERGLEICH' : 'BOSS FIGHT // COMPARE';
  const playgroundLabel = lang === 'de' ? 'TRAINING // LIVE' : 'TRAINING ROOM // LIVE';
    ...
    switch (data.type) {
        ...
        return <div className="grid h-full min-h-[620px] items-center gap-10 lg:grid-cols-[1.15fr_.85fr]">
          <div className="animate-fadeIn">
            ...
            <div className="mt-10 flex flex-wrap gap-3 font-mono text-sm">
              <span className="border-2 border-indigo-800 bg-slate-950 px-4 py-2 text-slate-300">DIREKTER PROMPT</span>
              <span className="self-center text-fuchsia-400">→</span>
              <span className="border-2 border-indigo-800 bg-slate-950 px-4 py-2 text-slate-300">L2-METHODE</span>
              <span className="self-center text-fuchsia-400">→</span>
              <span className="border-2 border-indigo-800 bg-slate-950 px-4 py-2 text-slate-300">L1-AUFTRAG</span>
              <span className="self-center text-fuchsia-400">→</span>
              <span className="border-2 border-emerald-700 bg-emerald-950/40 px-4 py-2 text-emerald-300">VERIFIED</span>
            </div>
          </div>
          <div className="retro-panel relative hidden min-h-[500px] overflow-hidden bg-[#080d20] p-8 lg:block">
            ...
            <div className="relative flex h-full min-h-[430px] flex-col justify-between">
              ...
              <div className="border-2 border-indigo-800 bg-slate-950 p-5 font-mono text-sm leading-7 text-emerald-300"><div>&gt; KLARHEIT</div><div>&gt; KONTEXT</div><div>&gt; EVIDENZ</div><div>&gt; GRENZEN</div><div className="pixel-pulse text-cyan-300">&gt; _</div></div>
            </div>
          </div>
        </div>;
      ...
      case SlideType.END:
        return <div className="flex h-full flex-col animate-fadeIn">
          <div className="mb-7 flex items-center gap-5 border-b-2 border-indigo-900 pb-5"><div className="border-2 border-fuchsia-700 bg-fuchsia-950/50 p-3 text-cyan-300 shadow-[4px_4px_0_#020617]"><IconComponent size={32} /></div><div><div className="pixel-font mb-2 text-[8px] text-fuchsia-400">{contentLabel}</div><h2 className="text-3xl font-black uppercase tracking-tight text-white md:text-5xl">{title}</h2></div></div>
      ...
      case SlideType.COMPARISON:
        return <div className="flex h-full flex-col animate-fadeIn"><div className="mb-4 flex items-center gap-4"><div className="border-2 border-fuchsia-700 bg-fuchsia-950/50 p-2 text-cyan-300"><IconComponent size={28} /></div><div><div className="pixel-font mb-2 text-[8px] text-fuchsia-400">{compareLabel}</div><h2 className="text-3xl font-black uppercase text-white md:text-4xl">{title}</h2></div></div>{subtitle && <p className="mb-5 text-lg font-semibold text-cyan-200">{subtitle}</p>}<div className="flex-grow"><PromptComparison standard={(lang === 'de' && data.codeStandardDE ? data.codeStandardDE : data.codeStandard) ?? ''} optimized={(lang === 'de' && data.codeOptimizedDE ? data.codeOptimizedDE : data.codeOptimized) ?? ''} technique={technique ?? (lang === 'de' ? 'Praxisfall' : 'Example')} description={typeof content === 'string' ? content : ''} lang={lang} workOrder={lang === 'de' && data.codeWorkOrderDE ? data.codeWorkOrderDE : data.codeWorkOrder} /></div></div>;
      case SlideType.PLAYGROUND:
        return <div className="flex h-full flex-col animate-fadeIn"><div className="mb-4 flex items-center gap-4"><div className="border-2 border-fuchsia-700 bg-fuchsia-950/50 p-2 text-cyan-300"><IconComponent size={28} /></div><div><div className="pixel-font mb-2 text-[8px] text-fuchsia-400">{playgroundLabel}</div><h2 className="text-3xl font-black uppercase text-white md:text-4xl">{title}</h2></div></div>{subtitle && <p className="mb-5 text-lg font-semibold text-cyan-200">{subtitle}</p>}<InteractivePlayground lang={lang} /></div>;
